...
import com.leaf.Secondhand.bean.User;
import com.leaf.Secondhand.util.UserDbHelper;
        ...
        btnLogin.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(CheckInput()) {
                    ...
                    for(User user : users) {
                        String enteredPasswordHash = PasswordHasher.hashPassword(EtStuPwd.getText().toString());
                        //如果可以找到,则输出登录成功,并跳转到主界面
                        if(user.getUsername().equals(EtStuNumber.getText().toString()) && user.getPassword().equals(enteredPasswordHash) ) {
                            flag = true;
                            Toast.makeText(LoginActivity.this,"恭喜你登录成功!",Toast.LENGTH_SHORT).show();
                            // 判断是否是管理员账号
                            if (user.getUsername().equals("21302010006") && user.getPassword().equals(enteredPasswordHash)) {
                                // 如果是管理员账号，跳转到管理员界面
